...
namespace
{
	...
	void PushPresence()
	{
		...
		activity.SetDetails(details.empty() ? std::string("Playing on EmuCoreC") : details);
		if (!state.empty())
		...
		if (!cover.empty())
		{
			...
			assets.SetLargeText(details.empty() ? std::string("EmuCoreC") : details);
...
Java_com_sbro_emucorec_discord_DiscordNative_available(JNIEnv*, jclass)
{
	return JNI_TRUE;
}

JNIEXPORT void JNICALL
Java_com_sbro_emucorec_discord_DiscordNative_start(JNIEnv* env, jclass, jstring saved_token)
{
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorec_discord_DiscordNative_authorize(JNIEnv*, jclass)
{
	std::shared_ptr<discordpp::Client> client;
	{
		std::lock_guard<std::mutex> lock(S().mutex);
...
Java_com_sbro_emucorec_discord_DiscordNative_takeToken(JNIEnv* env, jclass)
{
	std::string token;
	{
		std::lock_guard<std::mutex> lock(S().mutex);
...
Java_com_sbro_emucorec_discord_DiscordNative_status(JNIEnv*, jclass)
{
	return S().status.load(std::memory_order_acquire);
}

JNIEXPORT jstring JNICALL
Java_com_sbro_emucorec_discord_DiscordNative_error(JNIEnv* env, jclass)
{
	std::lock_guard<std::mutex> lock(S().mutex);
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorec_discord_DiscordNative_setPresence(
...
JNIEXPORT jstring JNICALL
Java_com_sbro_emucorec_discord_DiscordNative_self(JNIEnv* env, jclass)
{
	RefreshSelf();
	std::lock_guard<std::mutex> lock(S().mutex);
...
JNIEXPORT jstring JNICALL
Java_com_sbro_emucorec_discord_DiscordNative_friends(JNIEnv* env, jclass)
{
	std::shared_ptr<discordpp::Client> client;
	{
		std::lock_guard<std::mutex> lock(S().mutex);
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorec_discord_DiscordNative_pump(JNIEnv*, jclass)
{
	discordpp::RunCallbacks();
}

JNIEXPORT void JNICALL
Java_com_sbro_emucorec_discord_DiscordNative_stop(JNIEnv*, jclass)
{
	{
		std::lock_guard<std::mutex> lock(S().mutex);
		S().client.reset();
