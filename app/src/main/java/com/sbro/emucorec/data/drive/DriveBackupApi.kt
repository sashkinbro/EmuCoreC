...
import com.sbro.emucorec.data.drive.DriveBackupArchive.Companion.hex
...
class DriveBackupApi(private val context: Context, private val email: String) {
    ...
    suspend fun list(): List<DriveBackupEntry> {
        ...
        var next = ""
        do {
            val query = "trashed = false and appProperties has { key='emucorecBackup' and value='1' } and appProperties has { key='complete' and value='1' }"
    ...
    private suspend fun folder(): String {
        val q = "trashed = false and mimeType = 'application/vnd.google-apps.folder' and appProperties has { key='emucorecFolder' and value='1' }"
        ...
        return json("files?fields=id", "POST", JSONObject().put("name", "EmuCoreC Backups")
            .put("mimeType", "application/vnd.google-apps.folder")
            .put("appProperties", JSONObject().put("emucorecFolder", "1"))).getString("id")
    }
    ...
    suspend fun upload(snapshot: DriveSnapshot, verifying: suspend () -> Unit = {}, progress: suspend (Long, Long) -> Unit): String {
        ...
        val props = JSONObject().put("emucorecBackup", "1").put("complete", "0")
        ...
        if (url.isBlank()) {
            val metadata = JSONObject().put("id", id).put("name", "EmuCoreC-${snapshot.manifest.getLong("createdAt")}.zip.uploading")
        ...
        json("files/${enc(id)}", "PATCH", JSONObject().put("appProperties", props.put("complete", "1"))
            .put("name", "EmuCoreC-${snapshot.manifest.getLong("createdAt")}.zip"))
        sessionFile.delete()
        return id
    }
