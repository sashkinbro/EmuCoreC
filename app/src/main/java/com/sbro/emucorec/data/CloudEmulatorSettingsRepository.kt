...
class CloudEmulatorSettingsRepository(context: Context) {
    ...
    private val settingsRepository = SettingsBackupRepository(
        appContext,
        preferences,
        Ps3CoreConfigRepository(appContext),
    ...
    private fun exportPerGameSettings(): JSONObject {
        ...
        Ps3GameSettingsRepository(appContext).exportAll().toSortedMap()
            .entries.take(MAX_PER_GAME_FILES)
            .forEach { (name, content) -> files.put(name, content.take(MAX_PER_GAME_FILE_CHARS)) }
    ...
    private fun importPerGameSettings(root: JSONObject) {
        ...
        val entries = buildMap {
            files.keys().forEach { name ->
                ...
                if (content.isNotBlank()) put(name, content)
            }
        }
        Ps3GameSettingsRepository(appContext).importAll(entries)
    }
