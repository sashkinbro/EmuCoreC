...
/**
 * Keeps PS3 trophy progress (config files and TROPUSR.DAT under dev_hdd0/home/<user>/trophy)
 * on the signed-in account so a reinstall or a second device can recover unlocked trophies
 * after the game is installed again.
 */
class TrophyCloudRepository(context: Context) {
    ...
    private fun restoreDocument(commId: String, document: DocumentSnapshot): Boolean {
        ...
        val targetDir = File(localTrophyRoot(), commId)
        var written = false
        files.forEach { raw ->
    ...
    /** RPCS3 keeps config files and TROPUSR.DAT together in dev_hdd0/home/<user>/trophy/<NPWR...>. */
    private fun trophyRoots(): List<File> = TrophyRepository().trophyRoots(appContext)

    private fun localTrophyRoot(): File {
        val roots = trophyRoots()
        return roots.firstOrNull { it.parentFile?.name == DEFAULT_USER_ID }
            ?: roots.firstOrNull()
            ?: defaultTrophyRoot().also { it.mkdirs() }
    }

    private fun defaultTrophyRoot(): File {
        val home = EmulatorStorage.hdd0SaveDataRoot(appContext, DEFAULT_USER_ID).parentFile
            ?: File(EmulatorStorage.ps3Root(appContext), "config/dev_hdd0/home/$DEFAULT_USER_ID")
        return File(home, "trophy")
    }
