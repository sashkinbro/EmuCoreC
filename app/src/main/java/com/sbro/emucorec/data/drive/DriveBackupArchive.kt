...
class DriveBackupArchive(private val context: Context) {
    ...
    private val settingsRepository = SettingsBackupRepository(
        context,
        preferences,
        Ps3CoreConfigRepository(context),
    ...
    private fun roots(): Map<String, File> {
        ...
        val homeRoot = File(EmulatorStorage.ps3Root(context), "config/dev_hdd0/home")
        return linkedMapOf(
            ...
            "save-data" to homeRoot,
            "trophies" to homeRoot
        )
    }
    ...
    private fun allowed(name: String): Boolean {
        ...
        return when (parts[0]) {
            ...
            "save-data" -> parts.size >= 3 && parts[1].matches(Regex("[A-Za-z0-9]{1,16}")) && parts[2] == "savedata"
            "trophies" -> parts.size >= 3 && parts[1].matches(Regex("[A-Za-z0-9]{1,16}")) && parts[2] == "trophy"
            else -> false
        }
    }
    ...
    companion object {
        const val FORMAT = "emucorec-drive-backup"
