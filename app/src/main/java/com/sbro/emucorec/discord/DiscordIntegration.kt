package com.sbro.emucorec.discord
...
object DiscordIntegration {
    ...
    fun setPlaying(title: String, serial: String?) {
        ...
        gamePaused = false
        scope.launch {
            gameCoverUrl = withContext(Dispatchers.IO) {
                runCatching {
                    val catalog = com.sbro.emucorec.data.Ps3CatalogRepository(appContext)
                    val cover = catalog.findBySerial(serial.orEmpty())?.coverUrl
                        ?: catalog.findBestMatch(title)?.coverUrl
                    cover.orEmpty()
                }.getOrDefault("")
            }
            pushPresence()
    ...
    private const val TAG = "EmuCoreCDiscord"
